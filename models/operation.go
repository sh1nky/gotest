...
func (operationModel OperationModel) LockOperations(ids []string) (int64, error) {

	sqlUpdate := fmt.Sprintf("UPDATE operations SET status = \"%s\" WHERE id IN (%s)", StatusLocked, "\""+strings.Join(ids, "\", \"")+"\"")
